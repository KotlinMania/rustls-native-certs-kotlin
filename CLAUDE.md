# Claude Code Project Instructions — rustls-native-certs-kotlin

## Project Overview

This is **rustls-native-certs-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`rustls-native-certs`](https://crates.io/crates/rustls-native-certs).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/rustls-native-certs/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/rustls-native-certs>
