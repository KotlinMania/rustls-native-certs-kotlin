...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`rustls-native-certs`](https://crates.io/crates/rustls-native-certs).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `rustls-native-certs` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Load system trust roots into rustls
...
Original work copyrighted by the upstream `rustls-native-certs` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
